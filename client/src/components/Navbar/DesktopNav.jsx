...
const navs = [
  ...
  {
    url: "/edit-seating-chart",
    image: Classroom,
    text: "Edit",
    color: "grass",
  },
  {
    url: "/editneedsgoals/:teacherId/:classroomId",
    image: Goal,
    text: "Goals/Needs",
    color: "schoolBus",
  },
...
export default function DesktopNav({
  ...
  return (
    <>
      ...
      >
        <button
          className="absolute left-0 p-2 -mt-16 ml-5 w-14 bg-notebookPaper"
          onClick={toggle}
        ...
        <div className={`${isOpen ? "" : "hidden"} flex flex-col w-96 mt-20  `}>
          {navs.map((item, index) => (
            <motion.div
              ...
              className={`bg-${
                item.color
              } w-[30%] h-36 flex flex-col items-center border-[1rem] border-${
                item.color
              } rounded-tr-3xl rounded-br-3xl ${
                item.text === "Edit" && isEditMode ? "edit-mode" : ""
              } cursor-pointer`}
              onClick={() =>
                item.text === "Edit"
                  ? handleEditClick()
            ...
            >
              ...
              <div>
                <span className="font-poppins text-notebookPaper text-[12px]nav-text-shadow ">
                  {item.text}
                </span>
              </div>
            </motion.div>
