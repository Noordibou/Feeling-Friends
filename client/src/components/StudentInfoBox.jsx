  ...
  return (
    <>
      <div className="flex flex-col h-full w-full" key={`${student.id}`}>
        {/* bg object */}
        <div className={`${bgColorClass ? `bg-${bgColorClass} border-${bgColorClass}` : "bg-[#ece6d2] border-sandwich"} border-4 shadow-2xl p-4 h-full rounded-lg flex flex-row`}>
          {/* student image */}
          <div className={`flex h-full my-auto ${bgColorClass ? "w-full" : "w-28 opacity-70"}`}>
            <img
              src={avatarImg}
          ...
          <div className="flex flex-col w-[80%] px-4">
            ...
            {/* goals & needs */}
            <div className={`${ bgColorClass ? "bg-notebookPaper py-2 flex justify-around": "w-full"} px-2 rounded-md `} >
              {lastCheck ? 
                ...
                <div className="flex flex-row">
                  <h5 className="font-[Poppins]">{student.firstName} {student.lastName} hasn't checked in today.</h5>
                </div>
              }
              
              <div className={`flex items-center underline text-md ${bgColorClass ? "w-24" : "w-full flex justify-end h-full pt-4"} px-2`}>
