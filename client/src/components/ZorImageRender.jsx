...
const ZorImageRender = ({ sliderValue, chosenSubEmotion }) => {
  ...
  return (
    <div className="w-full mt-14">
        ...
        <h6 className="text-sm xl:text-lg font-normal w-3/12">I'm feeling {zone === "readytolearn" ? "ready to learn" : zone} !</h6>
        {girlImage && <img src={girlImage} alt="Girl Zor Image" className="h-52 xl:h-60 self-end mx-3" />}
        {emotionImagePath && <img src={emotionImagePath} alt="Emotion Image" className="h-36 xl:h-48 self-end mx-3" />}
        <h6 className="text-sm sm:text-xl xl:text-lg font-normal w-1/4 mt-10">Me too!</h6>
        </div>
    </div>
  );
};
